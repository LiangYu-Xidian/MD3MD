...
def _visualize(samples, dataset_info, max_num=25, wandb=None, spheres_3d=False):
    i=0
    for sample in samples:
        i=i+1
        print(i)
        positions, atom_type= sample['pos'],sample['atom_type']

        dists = torch.cdist(positions.unsqueeze(0), positions.unsqueeze(0)).squeeze(0)
        dists = dists[dists > 0]
        print("Average distance between atoms", dists.mean().item())
        plot_data3d(positions, atom_type, dataset_info=dataset_info, save_path=f'vis/{i}.png',
                    spheres_3d=spheres_3d)
