...
def get_classifier(dir_path='', device='cpu'):
    exp_path = '/exp_class_' + args.context[0] + '_gpu'
    dir_path += exp_path
    ...
    classifier_state_dict = torch.load(join(dir_path, 'best_checkpoint.npy'), map_location=torch.device(device))
...
def sample(args, device, model, dataset_info, prop_dist, nodesxsample, context):
    ...
    atom_list = unbatch(F.one_hot(torch.argmax(atom_type[:,:-1], dim=1), 5), batch.batch)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset', type=str, default='qm9',
                    help='qm9, geom')
    parser.add_argument('--ckpt', type=str, default='trained_model/generation',help='path for loading the checkpoint')
    parser.add_argument('--classifiers_path', type=str, default='trained_model/property_prediction')
    ...
    parser.add_argument('--w_global_pos', type=list,default=[1.0, 5.0, 5.0, 5.0, 5.0],
                    help='weights for global pos gradients')
    parser.add_argument('--w_global_node', type=list,default=[1.0, 6.0, 6.0,6.0,6.0],
                    help='weights for global node gradients')
    ...
    #args.classifiers_path = 'trained_model/property_prediction'
    ...
    logger.info('w_global_pos: %s' % ', '.join(map(str, args.w_global_pos)))
    logger.info('w_global_node: %s' % ', '.join(map(str, args.w_global_node)))
    ...
    print("We evaluate the classifier on our generated samples")
