...
from ..common import MultiLayerPerceptron, assemble_atom_pair_feature, extend_to_radius_graph
...
class MDMFullDP(nn.Module):
    ...
    def __init__(self, config):
        super().__init__()
        self.config = config
        self.num_scales=len(self.config.cutoff_list)
        """
        ...
        self.edge_encoder_global_list = nn.ModuleList([
            get_edge_encoder(config) for _ in range(self.num_scales)
        ])
        ...
        self.cutoff_list=config.cutoff_list
        #print('config:',config)
        #print('self_context_0:',self.context)
        ...
        # EGNN
        self.encoder_global_list = nn.ModuleList([
            EGNNSparseNetwork(
        ...
        """
        self.grad_global_dist_mlp_list = nn.ModuleList([
        ...
        self.grad_global_node_mlp_list = nn.ModuleList([
            MultiLayerPerceptron(
                1 * config.hidden_dim,
        ...
        self.model_global = nn.ModuleList([
            self.edge_encoder_global_list,
            self.encoder_global_list,
            self.grad_global_dist_mlp_list
        ])
    ...
    def net(self, atom_type, pos, bond_index, bond_type, batch, time_step,
            edge_index=None, edge_type=None, edge_length=None, context=None, vae_noise=None,arr_cutoff=['0','1000']):
        """
        Args:
        ...
        """
        #print(atom_type[-1].size())
        # N = atom_type.size(0)
        i=int(int(arr_cutoff[0])/2)
        ...
        # Encoding global
        edge_attr_global = self.edge_encoder_global_list[i](
            edge_length=edge_length,
            edge_type=edge_type
        )  # Embed edges
        if self.time_emb:
        ...
        node_attr_global, _ = self.encoder_global_list[i](
            z=atom_type,
            pos=pos,
            edge_index=edge_index,
            edge_attr=edge_attr_global,
        ...
        """
        dist_score_global = self.grad_global_dist_mlp_list[i](h_pair_global) # (E_global, 1)
        ...
        node_score_global = self.grad_global_node_mlp_list[i](node_attr_global)
    ...
    def forward(self, batch, context=None, return_unreduced_loss=False, return_unreduced_edge_loss=False,
        ...
        loss_all = 0
        loss_global_all = 0
        loss_node_global_all = 0
        vae_kl_loss_all = 0
        for i in range(len(self.cutoff_list)):
            arr_cutoff = self.cutoff_list[i].split("-")
            ...
            if self.vae_context:
                dist_score_global, _, node_score_global, _, \
        ...
        if return_unreduced_edge_loss:
            ...
            if self.vae_context:
                return loss_all, loss_global_all, loss_node_global_all, vae_kl_loss_all
            return loss_all, loss_global_all, loss_node_global_all
        else:
            return loss_all
    ...
    def langevin_dynamics_sample(self, atom_type, pos_init, bond_index, bond_type, batch, num_graphs, context,
                                 ...
                                 global_start_sigma=float('inf'), local_start_sigma=float('inf'), w_global_pos=[1.0,5.0,5.0,5.0,5.0],
                                 w_global_node=[1.0,6.0,6.0,6.0,6.0], w_local_pos=1, w_local_node=1, **kwargs):
        ...
        with torch.no_grad():
            ...
            for i, j in tqdm(zip(reversed(seq), reversed(seq_next)), desc='sample'):
                ...
                eps_pos=torch.zeros_like(pos_init)
                eps_node=torch.zeros_like(atom_type)
                for k in range(len(self.cutoff_list)):
                    arr_cutoff = self.cutoff_list[k].split("-")
                    net_out = self.net(
                        atom_type=atom_type,
                        pos=pos,
                        bond_index=bond_index,
                    ...
                    if self.vae_context:
                        edge_inv_global, _, node_score_global, _, \
                        edge_index, _, edge_length, local_edge_mask = net_out[:-1]
                    else:
                        edge_inv_global, _, node_score_global, _, \
                        edge_index, _, edge_length, local_edge_mask = net_out
                    ...
                    if sigmas[i] < global_start_sigma:
                        ...
                        node_score_global = 0
                    # Sum
                    eps_pos = eps_pos + w_global_pos[k] * node_eq_global  # + eps_pos_reg * w_reg
                    eps_node = eps_node + w_global_node[k] * node_score_global
                ...
                # charge
                atom_type_t = atom_type_t[:, :-1]
                charge_t = atom_type_t[:, -1]
                
                atom_type_t = torch.argmax(atom_type_t, dim=1)
                #if show_detail:
                #print("g pos:", pos_t)
                #print("generated element:", atom_type_t)
    ...
    def langevin_dynamics_sample_chain(self, atom_type, pos_init, bond_index, bond_type, batch, num_graphs, context,
                                 n_steps=100, step_lr=0.0000010, clip=1000, clip_local=None, clip_pos=None,
                                 global_start_sigma=float('inf'), local_start_sigma=float('inf'), w_global_pos=[1.0,5.0,5.0,5.0,5.0],
                                 w_global_node=[1.0,6.0,6.0,6.0,6.0], w_local_pos=1, w_local_node=1,show_detail=True, **kwargs):
