...
def extend_to_radius_graph(pos, edge_index, edge_type, cutoff, batch, unspecified_type_number=-1):

    #assert edge_type.dim() == 1
    N = pos.size(0)
    bgraph_edge_index = radius_graph(pos, r=int(cutoff[1]), batch=batch,max_num_neighbors=256)  # (2, E_r)
    #bgraph_edge_index = radius_graph(pos, r=int(cutoff[1]), batch=batch)  # (2, E_r)
    
    bgraph_adj = torch.sparse.LongTensor(
        bgraph_edge_index,
        torch.ones(bgraph_edge_index.size(1)).long().to(pos.device),
        torch.Size([N, N])
    )

    rgraph_edge_index = radius_graph(pos, r=int(cutoff[0]), batch=batch,max_num_neighbors=256)  # (2, E_r)
